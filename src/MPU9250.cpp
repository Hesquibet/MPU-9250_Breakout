...
MPU9250::MPU9250(uint8_t cspin){
  ...
  // if used with sparkfun breakout board https://www.sparkfun.com/products/13762 , change the pre-soldered JP2 to enable SPI (solder middle and left instead of middle and right)
  // pads are very small and re-soldering can be very tricky. I2C highly recommended.
  _csPin=cspin;
  SPI.begin();
...
void MPU9250::writeByte(uint8_t deviceAddress, uint8_t registerAddress, uint8_t data)
{
  if(_csPin!=-1){
...
void MPU9250::writeByteWire(uint8_t address, uint8_t subAddress, uint8_t data){
...
uint8_t MPU9250::readByteWire(uint8_t address, uint8_t subAddress){
